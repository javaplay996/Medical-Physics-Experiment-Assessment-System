    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑实验排课</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑实验排课</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">实验排课信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>课程类型</label>
                                        <select id="shiyanpaikeTypesSelect" name="shiyanpaikeTypes" class="form-control">
                                        </select>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>上课时间</label>
                                        <input id="shiyanpaikeTime-input" name="shiyanpaikeTime" type="text" class="form-control layui-input">
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
           ...
           if($("#jiaoshiId") !=null){
            ...
            httpJson("shiyanpaike/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addshiyanpaike');
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationShiyanpaiketypesSelect(){
            var shiyanpaikeTypesSelect = document.getElementById('shiyanpaikeTypesSelect');
            if(shiyanpaikeTypesSelect != null && shiyanpaikeTypesOptions != null  && shiyanpaikeTypesOptions.length > 0 ){
                for (var i = 0; i < shiyanpaikeTypesOptions.length; i++) {
                    shiyanpaikeTypesSelect.add(new Option(shiyanpaikeTypesOptions[i].indexName,shiyanpaikeTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(shiyanpaikeTypesSelect != null && shiyanpaikeTypesOptions != null  && shiyanpaikeTypesOptions.length > 0 ) {
            for (var i = 0; i < shiyanpaikeTypesOptions.length; i++) {
                if (shiyanpaikeTypesOptions[i].codeIndex == ruleForm.shiyanpaikeTypes) {//下拉框value对比,如果一致就赋值汉字
                        shiyanpaikeTypesSelect.options[i].selected = true;
                }
            }
        }
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                shiyanpaikeName: "required",
                shiyanpaikeTypes: "required",
                shiyanpaikeTime: "required",
                jiaoshiId: "required",
                banjiTypes: "required",
                shiyanpaikeFenzu: "required",
            },
            messages: {
                shiyanpaikeName: "课程名称不能为空",
                shiyanpaikeTypes: "课程类型不能为空",
                shiyanpaikeTime: "上课时间不能为空",
                jiaoshiId: "上课教师不能为空",
                banjiTypes: "上课班级不能为空",
                shiyanpaikeFenzu: "分组不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addshiyanpaike = window.sessionStorage.getItem("addshiyanpaike");
        if (addshiyanpaike != null && addshiyanpaike != "" && addshiyanpaike != "null") {
            window.sessionStorage.removeItem('addshiyanpaike');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("shiyanpaike/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#shiyanpaikeTime-input").val(ruleForm.shiyanpaikeTime);
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationShiyanpaiketypesSelect();
